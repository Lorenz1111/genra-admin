...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (savedLockoutTime) {
      const now = Date.now();
      if (now < savedLockoutTime) setLockoutTime(savedLockoutTime);
      else resetAttempts();
    }
  }, []);
    ...
    if (lockoutTime) {
      interval = setInterval(() => {
        ...
        if (timeRemaining <= 0) {
          ...
          resetAttempts();
        } else setCountdown(timeRemaining);
      }, 1000);
    }
  ...
  return (
    <Box sx={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, backgroundColor: '#fff' }}>
      <Box sx={{ position: 'absolute', top: 20, left: { xs: 20, md: 40 }, zIndex: 50 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate('/')} sx={{ color: '#64748b', textTransform: 'none', fontWeight: 600, fontSize: '0.95rem', '&:hover': { color: '#0f172a', backgroundColor: 'transparent' } }}>Back to Home</Button>
      </Box>

      <Box sx={{ width: { xs: '100%', md: '45%', lg: '40%' }, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', px: { xs: 4, sm: 8 }, boxShadow: { md: '10px 0 30px rgba(0,0,0,0.03)' }, zIndex: 10, backgroundColor: '#fff' }}>
        <Box sx={{ width: '100%', maxWidth: 380 }}>
          ...
          <Typography component="h1" variant="h4" sx={{ fontWeight: '900', color: '#0f172a', mb: 1, letterSpacing: '-0.02em' }}>Welcome Back</Typography>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Button fullWidth variant="outlined" startIcon={<GoogleIcon sx={{ color: '#db4437' }} />} onClick={() => handleOAuthLogin('google')} disabled={loading || isLockedOut} sx={{ py: 1.2, borderRadius: 2, borderColor: '#cbd5e1', color: '#0f172a', fontWeight: 'bold', textTransform: 'none', '&:hover': { backgroundColor: '#f8fafc', borderColor: '#94a3b8' } }}>Google</Button>
            <Button fullWidth variant="outlined" startIcon={<FacebookIcon sx={{ color: '#1877f2' }} />} onClick={() => handleOAuthLogin('facebook')} disabled={loading || isLockedOut} sx={{ py: 1.2, borderRadius: 2, borderColor: '#cbd5e1', color: '#0f172a', fontWeight: 'bold', textTransform: 'none', '&:hover': { backgroundColor: '#f8fafc', borderColor: '#94a3b8' } }}>Facebook</Button>
          </Box>
        </Box>
      </Box>

      <Box sx={{ display: { xs: 'none', md: 'flex' }, width: { md: '55%', lg: '60%' }, height: '100%', background: 'linear-gradient(135deg, #f8fafc 0%, #e0e7ff 100%)', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', p: 8 }}>
        <Box sx={{ zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', maxWidth: 480 }}>
          ...
          <Typography variant="body1" sx={{ color: '#475569', fontSize: '1.1rem', lineHeight: 1.6 }}>The central hub for authors to manage e-books and for administrators to oversee the GenrA reading ecosystem.</Typography>
        </Box>
      </Box>
    </Box>
  );
